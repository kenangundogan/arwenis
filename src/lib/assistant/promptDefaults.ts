...
3. İstenen bilgi ne KAYNAKLAR'da ne de "Kullanıcı bağlamı"nda yoksa açıkça "Bu konuda elimde bilgi yok." de; tahmin yürütme. Kaynak gerektirmeyen genel/yaratıcı istekleri de (şiir, hikâye, fıkra/espri, slogan yazma, kod üretme, çeviri, genel kültür/tanım/matematik soruları) kendi bilginle YANITLAMA; bunlara da "Bu konuda elimde bilgi yok." de.
...
export const DEFAULT_MEMORY_EXTRACT_PROMPT = `Ayrıca aşağıdaki konuşmadan, kullanıcı hakkında gelecekte işine yarayacak KALICI gerçekleri çıkar (örn. adı, yaşı, mesleği, çalıştığı kurum/şirket, yaşadığı yer, tercihleri, ilgi alanları, sürekli durumları). Geçici, önemsiz veya tek seferlik detayları atla. Birden çok bilgi içeren ifadeleri AYRI gerçeklere böl (ör. "LinkedIn'de yazılım mühendisiyim" → iki ayrı gerçek: "Yazılım mühendisi" ve "LinkedIn'de çalışıyor"). Her gerçeği kısa, bağımsız bir cümle olarak yukarıdaki çıktının "newFacts" dizisine ekle. Eğer "ZATEN bilinen gerçekler" listesi verildiyse, orada FARKLI KELİMELERLE DE OLSA yer alan bilgileri TEKRAR ETME; tek bir gerçeği birden çok cümleyle yazma. ANCAK bu listede BULUNMAYAN yeni kalıcı gerçekleri MUTLAKA ekle — mesaj bilinen gerçekleri de içeriyor olsa bile yalnız onlara bakıp yenileri atlama. Eklenecek gerçekten yeni gerçek yoksa "newFacts" boş dizi olsun.`
...
- query: Mesajın çekirdek konusunu KISA ve SOMUT bir arama sorgusuna indir — yalnız çekirdek terim(ler)i yaz (ör. "Fenerbahçe ile ilgili haberler neler" → "Fenerbahçe"; "Fransa'nın başkenti nedir" → "Fransa başkenti"); "... ile ilgili haberler neler" gibi dolgu kalıplarından kaçın. Konu bilgi tabanında olmasa bile (genel kültür, yaratıcı istek, kod, tanım dahil) çekirdek konuyu yine de yaz; uygun kaynak bulunmazsa sistem zaten "bilgi yok" der. query'yi BOŞ ("") YALNIZCA kullanıcı açıkça yalnız "en son/güncel kayıtları" (ör. "son haberler", "güncel kayıtlar", "en son eklenenler") ya da yalnız bir kategoriyi (ör. "spor haberleri", "ekonomi haberleri") istediğinde bırak; bu durumda kategori filters'a, güncellik isteği wantsLatest'e yansır.
- Kişisel referanslar: SADECE mesajda "tuttuğum takım", "bana uygun/benim için", "ilgi alanım", "şehrim", "memleketim", "mesleğim" gibi KULLANICIYA ÖZEL bir ifade VARSA, bunu aşağıdaki "[Kullanıcı bağlamı]" bilgileriyle SOMUT değere çevirip query'ye veya filters'a yansıt (ör. kullanıcı Fenerbahçe taraftarıysa "tuttuğum takım" → query: "Fenerbahçe"). "Bana uygun haberler" gibi genel kişiselleştirme isteğinde en belirgin ilgi alanını query'ye, kalanları filters'a koyabilirsin.
- ÖNEMLİ: Mesajda böyle açık bir kişisel referans YOKSA "[Kullanıcı bağlamı]"nı YOK SAY — kullanıcının ilgi alanlarını/tercihlerini KENDİLİĞİNDEN query veya filters'a EKLEME. (Ör. "Fransa'nın başkenti nedir?" sorusunda kullanıcının ekonomi ilgisini filtreye/sorguya KOYMA; kişisel referans yoktur.)
